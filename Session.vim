let SessionLoad = 1
let s:so_save = &g:so | let s:siso_save = &g:siso | setg so=0 siso=0 | setl so=-1 siso=-1
let v:this_session=expand("<sfile>:p")
silent only
silent tabonly
cd ~/.local/source/ddstats-rust/ddstats-rust
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +1 /opt/st/config.h
badd +14 Cargo.toml
badd +61 ~/.Xresources
badd +44 .github/workflows/rust.yml
badd +1 test.sh
badd +1 test.sg
badd +3 src/main.rs
badd +83 src/threads.rs
badd +1 src/client.rs
badd +200 src/mem.rs
badd +21 src/consts.rs
badd +136 src/ui.rs
badd +60 src/config.rs
argglobal
%argdel
$argadd src/main.rs
set stal=2
tabnew
tabnew
tabnew
tabnew
tabnew
tabnew
tabrewind
edit src/ui.rs
argglobal
balt src/main.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 241 - ((47 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 241
normal! 0
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext
edit ~/.local/source/ddstats-rust/ddstats-rust/src/config.rs
argglobal
balt ~/.local/source/ddstats-rust/ddstats-rust/src/threads.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 39 - ((27 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 39
normal! 08|
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext
edit ~/.local/source/ddstats-rust/ddstats-rust/src/mem.rs
argglobal
balt ~/.local/source/ddstats-rust/ddstats-rust/src/client.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 1 - ((0 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 1
normal! 05|
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext
edit ~/.local/source/ddstats-rust/ddstats-rust/src/threads.rs
argglobal
balt ~/.local/source/ddstats-rust/ddstats-rust/src/mem.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 111 - ((42 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 111
normal! 024|
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext
edit ~/.local/source/ddstats-rust/ddstats-rust/src/config.rs
argglobal
balt ~/.local/source/ddstats-rust/ddstats-rust/src/mem.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 32 - ((14 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 32
normal! 06|
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext
edit ~/.local/source/ddstats-rust/ddstats-rust/src/consts.rs
argglobal
balt ~/.local/source/ddstats-rust/ddstats-rust/src/config.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 30 - ((22 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 30
normal! 0
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext
edit ~/.local/source/ddstats-rust/ddstats-rust/src/ui.rs
argglobal
balt ~/.local/source/ddstats-rust/ddstats-rust/src/config.rs
setlocal fdm=manual
setlocal fde=0
setlocal fmr={{{,}}}
setlocal fdi=#
setlocal fdl=0
setlocal fml=1
setlocal fdn=20
setlocal nofen
silent! normal! zE
let &fdl = &fdl
let s:l = 154 - ((42 * winheight(0) + 24) / 48)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 154
normal! 0
lcd ~/.local/source/ddstats-rust/ddstats-rust
tabnext 1
set stal=1
if exists('s:wipebuf') && len(win_findbuf(s:wipebuf)) == 0&& getbufvar(s:wipebuf, '&buftype') isnot# 'terminal'
  silent exe 'bwipe ' . s:wipebuf
endif
unlet! s:wipebuf
set winheight=1 winwidth=20 shortmess=filnxtToOF
let s:sx = expand("<sfile>:p:r")."x.vim"
if filereadable(s:sx)
  exe "source " . fnameescape(s:sx)
endif
let &g:so = s:so_save | let &g:siso = s:siso_save
nohlsearch
doautoall SessionLoadPost
unlet SessionLoad
" vim: set ft=vim :
